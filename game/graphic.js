function init()
{
    ...
    player1 = new Player("player1", 0xffff00, new THREE.Vector2(50, 0), 0, 3);
    scene.add(player1.graphic);

    enemy = new Player("enemy", 0xffff00, new THREE.Vector2(50, 0), 0, 3);
    ...
    for (x = minX; x <= maxX; x = x+sizeOfTileX){
        for (y = minY; y <= maxY; y = y+sizeOfTileY){
            ...
            if (x == 0 && y == 0) {
                ...
                new THREE.MeshLambertMaterial({color: 0x19f29e, transparent: true, opacity: 0.6}));
                tmpGround.position.x = x;
                tmpGround.position.y = y;
                scene.add(tmpGround); 
            }
            else if (0x000000 != color && (x != 0 || y != 0))
            {
                tmpGround = new THREE.Mesh(
                new THREE.PlaneGeometry(sizeOfTileX-10, sizeOfTileY-10),
...
{
    pointLight = new THREE.PointLight(color, 100, 100000);
